...
(function ($) {
  ...
  /* Add Wrap of iframe */
  $('.paoc-popup iframe[src*="vimeo.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="dailymotion.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="youtube.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="m.youtube.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="youtu.be"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="screencast-o-matic.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="videopress.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="video.wordpress.com"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );
  $('.paoc-popup iframe[src*="fast.wistia.net"]').wrap(
    '<div class="paoc-iframe-wrap" />'
  );

  /* Zero the idle timer on mouse movement */
  $(document).on(
    "mousemove keypress scroll click touchstart touchmove",
    function () {
      paoc_ideal_timer = 0;
    }
  );
  ...
  $(".paoc-popup-page-load.paoc-popup-js").each(function (index) {
    ...
    if (typeof target !== "undefined") {
      ...
      var paoc_active_flag = popupaoc_popup_active_flag(
        index,
        target,
        data_opts,
        options
  ...
  $(document).on("click", '[class*="paoc-popup-cust-"]', function () {
    ...
    $.each(html_classes, function (class_key, class_val) {
      ...
      if (
        typeof popup_ele !== "undefined" &&
        (data_opts.popup_type == "simple_link" ||
          data_opts.popup_type == "button" ||
          data_opts.popup_type == "image")
      ) {
        var paoc_active_flag = popupaoc_popup_active_flag(
          class_key,
          popup_ele,
...
function popupaoc_open_popup(target, options, data_opts) {
  ...
  if (typeof popupaoc_popup_befoer_open === "function") {
    paoc_popup_open = popupaoc_popup_befoer_open(
      paoc_popup_open,
      target,
      options,
      data_opts
...
function popupaoc_set_popup_events(target, options, data_opts) {
  ...
  options.content.onOpen = function () {
    ...
    jQuery(".custombox-overlay, .custombox-content").removeClass(
      "paoc-popup-active"
    );
    ...
    jQuery(".custombox-overlay")
      ...
      .addClass(
        "paoc-popup-active paoc-popup-overlay paoc-popup-overlay-" +
          data_opts.id
      );
    ...
    jQuery(".custombox-content")
      .not(".paoc-cb-popup")
      .addClass(
        "paoc-popup-active paoc-cb-popup paoc-cb-popup-" +
          data_opts.id +
          " paoc-popup-" +
          options.content.positionX +
          "-" +
          options.content.positionY
      );

    if (
      options.overlay.active == false &&
      options.content.fullscreen == false
    ) {
      jQuery(".custombox-content.paoc-popup-active").addClass(
        "paoc-hide-overlay"
      );
    }
...
function popupaoc_popup_active_flag(index, $this, data_opts, options) {
  ...
  if (
    typeof data_opts.cookie_expire !== "undefined" &&
    data_opts.cookie_expire !== "" &&
    popupaoc_get_cookie_value(data_opts.cookie_prefix + "_" + data_opts.id) !=
      null
  ) {
    return 0;
  }
...
function popupaoc_create_cookie(name, value, time_val, type, samesite) {
  ...
  document.cookie =
    encodeURIComponent(name) + "=" + value + expires + "; path=/" + samesite;
}
